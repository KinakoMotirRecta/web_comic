...
**未公開の話を通知しないこと。** サイトは「次回公開予定」の話を先に一覧へ載せることがある。
ヤンマガWebでは未公開話にもタイトルとURLが入っており、公開済みかどうかは
`mod-episode-public` / `mod-episode-nopublic` の違いにしか出ない（2026-09-17 に誤通知した実績あり）。
新しい取得関数を書くときは、そのサイトが未公開話を露出しないか必ず確認する。
`load_episodes()` に「公開日が未来なら除外する」安全網があるが、
日付が読めない場合は素通りするので、これだけに頼らない。
