...
日本語の作品名（`100年留年してるエルフ`）でも、URLのエンコード済みの形（`100%E5%B9%B4...`）でもよい。
作者名はページから取らず `author` に手で書く（HTML構造への依存を減らすため）。

一覧は新着順の1ページ目（10話程度）だけを見る。更新検知には十分。

ヤンマガWebは「次回公開予定」の話も一覧に入れてくるので、公開済みのものだけを通知する。
...
| やりたいこと | cron |
|---|---|
| 毎日 JST 12:15 に1回（現在の設定） | `15 3 * * *` |
| JST 12:15 / 18:15 / 24:15 の3回 | `15 3,9,15 * * *` |
| 1時間おき | `15 * * * *` |

GitHub の cron は大きく遅れる。このリポジトリでは JST 12:15 指定に対し、実際は 16:50〜17:50 頃に動いている（2026年9月の実績）。時刻の精度は期待しない。
...
| タイミング | 動作 |
|---|---|
| **失敗した時点** | ⚠️ 失敗を通知。原因のエラーメッセージ付き |
| 翌日以降も失敗 | 直るまで **24時間おきに1回だけ**再通知 |
